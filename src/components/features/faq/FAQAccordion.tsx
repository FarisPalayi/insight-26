  ...
  return (
    ...
    >
      <Accordion
        type="single"
        collapsible
        className="space-y-0 md:space-y-2 rounded-md md:rounded-none overflow-hidden"
      >
        {faqs.map((faq, index) => (
          <AccordionItem
            key={index}
            value={`item-${index}`}
            className="glass-surface rounded-none md:rounded-lg border-0 px-5 md:px-6 overflow-hidden"
          >
